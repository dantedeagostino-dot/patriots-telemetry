...
export default function PatriotsDashboard() {
  ...
  const fetchIntel = useCallback(async () => {
    ...
    try {
      ...
      if (patsEvent) {
        ...
        // 2. PEDIR DATOS SEGÚN EL DOCUMENTO API OVERVIEW
        const [scoringRes, leaderRes, predRes, newsRes] = await Promise.all([
          ...
          // Intentamos líderes de 2024 para asegurar que carguen datos mientras 2025 se llena
          fetch(`https://nfl-api1.p.rapidapi.com/team/leaders?teamId=22&limit=3&season=2024`, options),
          // Usamos /nflpicks para las predicciones del PickCenter
        ...
        // CORRECCIÓN: Si el scoreboard ya tiene líderes del partido, úsalos. Si no, usa los de la temporada.
        const gameLeaders = comp.leaders?.[0]?.leaders || lData?.data || [];
        setTeamLeaders(gameLeaders);
        ...
        const scoreDiff = parseInt(patsTeam.score) - parseInt(comp.competitors.find((c: any) => c.team.abbreviation !== 'NE').score);
  ...
  if (loading) return <div className="min-h-screen bg-black flex flex-col items-center justify-center text-blue-500 animate-pulse font-mono"><Cpu size={64} className="mb-4" />CALIBRATING_INTEL_SYSTEM_v3.9...</div>;
    ...
    <main className="min-h-screen bg-[#000d16] text-slate-100 p-4 lg:p-6 font-mono overflow-x-hidden">
      ...
      <header className="flex justify-between items-center border-b border-blue-900/50 pb-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-black italic uppercase text-white tracking-tighter">Patriots_Command_v3.9</h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          <section className="bg-slate-950 border border-blue-900/30 p-6 shadow-2xl">
            ...
            <div className="h-56 w-full border border-blue-500/30 relative overflow-hidden rounded-sm" style={{backgroundImage: `url('${NFL_FIELD_TOP_DOWN}')`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
               <div className="absolute inset-0 bg-black/50 p-4 flex flex-col">
                 <div className="flex justify-between items-center mb-4">
                    <p className="text-[10px] text-white font-black uppercase flex items-center gap-2 drop-shadow-lg"><TrendingUp size={14} className="text-blue-400"/> Win_Probability_Stock</p>
            ...
            <div className="grid grid-cols-3 gap-4">
              {teamLeaders.length > 0 ? teamLeaders.map((leader: any, i: number) => (
                <div key={i} className="text-center bg-white/5 p-2 border-t border-yellow-600/30">
                  <p className="text-[9px] font-black text-white uppercase truncate">{leader.athlete?.displayName || leader.name || "Unit"}</p>
                  <p className="text-[8px] text-yellow-500 font-bold">{leader.displayValue || leader.position}</p>
                </div>
              )) : <div className="col-span-3 text-center text-[10px] text-slate-600 italic">Calibrating field metrics...</div>}
            </div>
          </section>
        </div>

        {/* UNIT SELECTOR & LEAGUE INTEL */}
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-blue-900/10 border border-blue-500/30 p-4 text-center">
             <p className="text-[9px] text-blue-400 font-black uppercase mb-1">PickCenter_Intel_v2</p>
             <p className="text-xl font-black text-white uppercase">
               {predictions?.pickCenter?.[0]?.details || predictions?.data?.lines?.[0]?.details || "PRED_LOCKED"}
             </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-12">
          <section className="bg-gradient-to-r from-slate-950 to-blue-900/30 border-t-2 border-red-600 p-6 flex flex-col md:flex-row items-center gap-10">
             ...
             <div className="flex gap-12 border-l border-slate-800 pl-12 text-center font-black uppercase">
                <div><p className="text-[9px] text-slate-500 italic mb-1">Weight</p><p className="text-4xl text-white">{playerBio?.displayWeight || '--'}</p></div>
                <div><p className="text-[9px] text-slate-500 italic mb-1">Status</p><p className="text-4xl text-green-500">ACT</p></div>
             </div>
          </section>
        </div>
      </div>
      <style jsx>{` @keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } } .animate-marquee { animation: marquee 30s linear infinite; } .custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #1e293b; border-radius: 10px; } `}</style>
    </main>
  );
}